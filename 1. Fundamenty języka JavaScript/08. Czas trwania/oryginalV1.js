...
const convertToSeconds = (time) => {
  ...
  const seconds = parseInt(splited[0]) * 60 + parseInt(splited[1]);
  return seconds;
};

const findModule = (moduleNumber) => {
  const pattern = `${moduleNumber}x`;
  const module = lessons.filter(({ name }) => name.match(pattern));
  return module;
};

const calculateDuration = (module) => {
  const time = module.reduce(
    (total, { length }) => (total += convertToSeconds(length)),
    0
  );
  return time;
};

const toHours = (time) => parseInt(time / 3600);

const toMinutes = (time) => parseInt((time % 3600) / 60);

const toSeconds = (time) => time % 60;

const formatDuration = (duration) =>
  `${toHours(duration)} godzin ${toMinutes(duration)} minut i ${toSeconds(
    duration
  )} sekund `;

const calculatePercentage = (fragment, total) =>
  ((fragment / total) * 100).toFixed(2);

const totalDuration = () => {
  const totalTime = calculateDuration(lessons);
  const module1 = calculateDuration(findModule(1));
  const module2 = calculateDuration(findModule(2));
  const module3 = calculateDuration(findModule(3));
  const info = `Kurs Opanuj JavaScript trwa ${formatDuration(totalTime)}.
     Moduł Adama: ${formatDuration(module1)} co stanowi ${calculatePercentage(
    module1,
    totalTime
  )}%
     Moduł Przemka: ${formatDuration(module2)} co stanowi ${calculatePercentage(
    module2,
    totalTime
  )}%
     Moduł Marcina: ${formatDuration(module3)} co stanowi ${calculatePercentage(
    module3,
    totalTime
  )}%`;
  return info;
